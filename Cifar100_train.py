...
parser.add_argument('--model', default='mlpmixer', type=str, help='choose the model')
...
parser.add_argument('--root', default='./cifar/cifar100', type=str, metavar='PATH', help='path to dataset')
parser.add_argument('--device', default='1', type=str, metavar='PATH', help='path to dataset')
parser.add_argument('--cls_nums', default=100, type=int, help='model to class numbers')
...
acc_lists = []
mdl_file = './record/checkpoint/cifar100/cifar100_{}.mdl'.format(args.model)
rec_file = './record/log/cifar100/cifar100_{}_log.txt'.format(args.model)
pho_file = './record/chart/cifar100/cifar100_{}_log.png'.format(args.model)
...
trainset = torchvision.datasets.CIFAR100(root=args.root, train=True, download=False, transform=transform_train)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=128, shuffle=True, num_workers=2)

testset = torchvision.datasets.CIFAR100(root=args.root, train=False, download=False, transform=transform_test)
...
logger.info("num_classes:{} num_blocks:{}".format(num_classes,num_blocks))
if args.model == 'mlpmixer':
    net = MlpMixer((8, 8), (32, 32), 3, 32, 32*2, 32*4, num_blocks, num_classes)
elif args.model == 's2mlpv1':
    net = S2MLPv1((8, 8), (32, 32), 3, 32, 4, num_blocks, num_classes)
elif args.model == 's2mlpv2':
    net = S2MLPv2((8, 8), (32, 32), 3, 32, 4, num_blocks, num_classes)
elif args.model == 'vip':
    net = ViP((8, 8), (32, 32), 3, 32, 8, 4, num_blocks, num_classes)
elif args.model == 'asmlp':
    net = ASMLP((8, 8), (32, 32), 3, 32, 4, num_blocks, num_classes)
elif args.model == 'cyclemlp':
    net = CycleMLP((8, 8), (32, 32), 3, 32, 4, num_blocks, num_classes)
elif args.model == 'hiremlp':
    net = HireMLP((8, 8), (32, 32), 3, 32, 4, num_blocks, num_classes)
elif args.model == 'sparsemlp':
    net = SparseMLP((8, 8), (32, 32), 3, 32, 4, num_blocks, num_classes)
elif args.model == 'convmlp':
    net = ConvMLP((8, 8), (32, 32), 3, 32, 4, num_blocks, num_classes)
elif args.model == 'gmlp':
    net = gMLP((8, 8), (32, 32), 3, 32, 4, num_blocks, num_classes)
elif args.model == 'amlp':
    net = aMLP((8, 8), (32, 32), 3, 32, 4, num_blocks, num_classes)
elif args.model == 'resmlp':
    net = ResMLP((8, 8), (32, 32), 3, 32, 4, num_blocks, num_classes)
elif args.model == 'repmlpnet':
    net = RepMLPNet((8, 8), (32, 32), 3, 32, 4, num_blocks, num_classes)
elif args.model == 'spinmlp':
    net = SpinMLP((8, 8), (32, 32), 3, 32, 4, num_blocks, num_classes, False)
else:
