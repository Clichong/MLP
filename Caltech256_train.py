...
parser.add_argument('--model', default='resnet50', type=str, help='choose the model')
...
parser.add_argument('--root', default='./caltech/caltech256/256_ObjectCategories/', type=str, metavar='PATH', help='path to dataset')
parser.add_argument('--device', default='1,0', type=str, metavar='PATH', help='path to dataset')
parser.add_argument('--cls_nums', default=256, type=int, help='model to class numbers')
...
trainiterloss_lists = []
testloss_lists = []
mdl_file = './record/checkpoint/caltech256/caltech256_{}.mdl'.format(args.model)
rec_file = './record/log/caltech256/caltech256_{}_log.txt'.format(args.model)
pho_file = './record/chart/caltech256/caltech256_{}_log.png'.format(args.model)
trainloss_imgfile = './record/loss/caltech256/caltech256_{}_trainloss.png'.format(args.model)
trainiterloss_imgfile = './record/loss/caltech256/caltech256_{}_trainiterloss.png'.format(args.model)
testloss_imgfile  = './record/loss/caltech256/caltech256_{}_testloss.png'.format(args.model)
...
if args.model == 'mlpmixer':
    ...
    net = StageMLP(drop=0.6, drop_path_rate=0.2, num_classes=257)
elif args.model == 'resnet50':
    net = torchvision.models.resnet50(pretrained=True)
    net.fc = nn.Linear(2048, 257)
else:
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        trainiterloss_lists.append(loss.item())
...
list_to_draw(trainiterloss_lists, filename=trainiterloss_lists, title='{}_trainiterloss'.format(args.model),
             xlabel='Epochs', ylabel='Train Loss')
list_to_draw(testloss_lists, filename=testloss_imgfile, title='{}_testloss'.format(args.model),
             xlabel='Epochs', ylabel='Test Loss')
print("Success Draw Photo")
